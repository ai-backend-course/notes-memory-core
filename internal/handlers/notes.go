...
// Note represents a single note record from the database.
type Note struct {
	ID        int       `json:"id"`
...
// HealthCheck provides a simple API health response.
func HealthCheck(c *fiber.Ctx) error {
	return c.JSON(fiber.Map{
		"status":  "ok",
		"message": "✅ Notes API is healthy and running!",
	})
}
...
func CreateNote(c *fiber.Ctx) error {
	...
	if n.Title == "" || n.Content == "" {
		return c.Status(http.StatusBadRequest).
			JSON(fiber.Map{"error": "title and content are required fields"})
	}
